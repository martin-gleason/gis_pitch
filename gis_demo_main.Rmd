...
```
---
###  Internvention Status Map: Decoded
```{r caluclations, include = TRUE, echo = TRUE, message = FALSE, warning = FALSE}

n_clients <- geo_coded_diversions_sf%>%
  filter(lubridate::year(STRDTE) >= 2016) %>%
  nrow()
```
* Filtering for the most recent cases gives us a more useable map
  *    *Code included on purpose*
* A total of `r n_clients` clients

* Partnering with GIS would allow for more real time analytics:
  *  Based on currenbt JEMS/CFive Data
...
---
# Other Sources
```{r heatmap, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
heat_map_CTA
```